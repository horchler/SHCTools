...
%   Andrew D. Horchler, horchler @ gmail . com, Created 3-12-12
...
if isMedianMode && nargin < 4 || isLikeRnd && nargin < 6 || nargin < 5
    error('SHCTools:stoneholmesargs:TooFewInputs',...
...
if ns(1) && (~isLike && any(sz([false;ns(2:end)],1) ~= sz(ns(1),1)) ...
    ...
    if isX && ns(1) && ~isLike
        ...
        errmsg = ['If more than one of ' thetastr ', Lambda_U, Lambda_S, ' ...
                  'or ' msgstr ' are non-scalar, they must have the same '...
                  'number of rows.'];
    elseif isMean
    ...
    elseif isLike
        if isTheta
            ...
            errmsg = ['Delta, Epsilon, Lambda_U, and Lambda_S must be '...
                      'scalars or 1-by-N-by-... arrays.'];
        end
    else
        if isTheta
...
if size(sza,1) > 1 && any(any(bsxfun(@ne,sza(2:end,2:end),sza(1,2:end)),2))
    ...
    if isX && isArray(1)
        ...
        elseif isLike
            ...
            switch(lower(str))
                ...
                case 'rnd'
                    msgstr = ['the specified dimensions are non-scalar '...
                              'arrays and not column vectors'];
                otherwise
                    msgstr = ['the data are non-scalar arrays and not '...
                              'column vectors'];
            end
        end
        errmsg = ['If more than one of ' thetastr ', Lambda_U, Lambda_S, ' ...
                  'or ' msgstr ', they must have the same dimensions.'];
    elseif isMean
        errmsg = ['If more than one of ' thetastr ', Lambda_U, or Lambda_S '...
    ...
    elseif isLike
        ...
        if isTheta
            errmsg = ['If Theta and/or Lambda_U are non-scalar arrays and '...
                      'not column vectors, they must have the same '...
                      'dimensions.'];
        else
            errmsg = ['If more than one of Delta, Epsilon, or Lambda_U are '...
    ...
    if isLike
        ...
        if isX
            ...
            if isMean && nargin == 4 || isMedianMode && nargin == 4
                expnd = [false;
                         (isCol & (1-eye(length(sz)))*double(isArray) > 0)];
            else
