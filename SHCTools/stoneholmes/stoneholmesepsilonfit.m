...
%   EPSILON = STONEHOLMESEPSILONFIT(EPSILON_HAT,TAU,TAU_BAR,LAMBDA_U) returns
%   the noise magnitudes EPSILON required to achieve desired periods TAU. The
%   system has unstable eigenvalues LAMBDA_U and measured mean periods TAU_BAR
%   when simulated at noise magnitudes of EPSILON_HAT. The inputs must be
%   scalars or equal length vectors, which are applied elementwise to the
%   output.
%   
%   NOTE:
