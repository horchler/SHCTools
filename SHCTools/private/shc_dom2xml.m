        ...
        if regexpi(dtd,pattern) ~= 1    	% First check if valid absolute URL
            if regexpi(dtd,filepart) ~= 1	% Check if valid relative URL
                error(  'SHCTools:shc_dom2xml:InvalidDTDLocation',...
                       ['Invalid DTD location. Either illegal characters '...
                        'were specified in the URI, or its format is '...
                        'neither a valid absolute nor relative URI.']);
            end
        end
        internalDTD = false;
        externalDTD = true;
...
if any(compareFields)
    ...
    elseif length(schema) > 4 && strcmpi(schema(end-3:end),'.xsd')
        ...
        error(  'SHCTools:shc_dom2xml:InvalidSchema',...
               ['Schema string option must be an empty string (no Schema) '...
                'or a valid URI for a ''.xsd'' file.']);
    end
else
    hasXSD = false;
end
...
if internalDTD
    writer.println(sprintf([...
        '<!DOCTYPE net\n'...
        '[\n'...
        '   <!ELEMENT net (contour|channel|cluster)>\n'...
        '   <!ELEMENT contour (contourfields,subnets?)>\n'...
        '   <!ELEMENT channel (channelfields,subnets?)>\n'...
        '   <!ELEMENT cluster (clusterfields,subnets?)>\n'...
        '   <!ELEMENT contourfields (parent?,node,size,alpha,beta?,gamma,delta,direction?)>\n'...
        '   <!ELEMENT channelfields (parent?,node,size,alpha,beta?,gamma,delta,direction?)>\n'...
        '   <!ELEMENT clusterfields (parent?,node,size,alpha,beta?,gamma,delta?,direction?)>\n'...
        '   <!ELEMENT subnets (contour|channel|cluster)*>\n'...
