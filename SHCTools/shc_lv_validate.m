function varargout=shc_lv_validate(rho,alpha,epsilon)
...
try
    ...
    if ~isreal(alpha) || ~all(isfinitesym(alpha(:)))
        error('SHCTools:shc_lv_validate:AlphaNonFiniteReal',...
             ['The input Alpha must be a finite real symbolic or '...
    ...
    if nargin > 2
        if ~isfloat(epsilon) || ~isreal(epsilon)
            error('SHCTools:shc_lv_validate:InvalidEpsilon',...
                 ['The input Epsilon must be a finite real floating-point '...
                  'vector.']);
        end
