function status=shc_sdeprint(t,y,flag,w)
%SHC_SDEPRINT  Command window printing SDE output function.
%   When the function SHC_SDEPRINT is passed to an SDE solver as the 'OutputFUN'
%   property, i.e., OPTIONS = SHC_SDESET('OutputFUN',@SHC_SDEPRINT), the solver
%   calls SHC_SDEPRINT(T,Y,'') after every timestep. The SHC_SDEPRINT function
%   prints the components of the solution, Y, as they are computed. If more than
...
%   If the solver's 'Refine' property is greater than one (see SHC_SDESET), T is
...
%   SHC_SDEPRINT(T,Y,'',W). If the 'OutputWSelect' property is set to 'yes' or a
%   non-empty vector of indices the solver calls
...
%   
%   See also:
%       SHC_SDEPLOT, SHC_SDEPHASEPLOT3, SHC_LV_INTEGRATE, SHC_SDESET,
%       SHC_SDEGET, ODEPRINT, FORMAT
...
switch flag
    case 'init'
        ...
        [lastmsg,lastid] = lastwarn('','SHCTools:shc_sdeprint:CatchWarning');
    ...
    case ''
        if isempty(LEN_TSPAN) || isempty(STR)
            if isW
                error('SHCTools:shc_sdeprint:NotCalledWithInitW',...
                     ['Output function has not been initialized. Use syntax '...
                      'OutputFUN(TSPAN,Y0,''init'',W0).']);
            else
                error('SHCTools:shc_sdeprint:NotCalledWithInit',...
                     ['Output function has not been initialized. Use syntax '...
        ...
        if ~strcmp(msgid,'SHCTools:shc_sdeprint:CatchWarning')
            % Reset state of lastwarn
            lastwarn('','SHCTools:shc_sdeprint:CatchWarning');
    ...
    otherwise
        error('SHCTools:shc_sdeprint:InvalidFlag',...
