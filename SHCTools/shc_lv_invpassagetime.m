...
if any(d_eta0 < d_etarng(1)) || any(d_eta0 > d_etarng(2)) || any(isnan(d_eta0))
...
switch lower(method)
    case {'default','quad'}
        f = @(d_eta,d,lambda_u,lambda_s,tp)etaroot(d_eta,d,lambda_u,lambda_s,...
            tp,options.TolX);
        msgid = 'MATLAB:quad:MinStepSize';
    case 'quadgk'
        f = @(d_eta,d,lambda_u,lambda_s,tp)etarootgk(d_eta,d,lambda_u,...
            lambda_s,tp,options.TolX);
...
if ~isempty(msgid)
    CatchWarningObj = catchwarning(msgid);	%#ok<NASGU>
end

for i = n:-1:1
    % Create function handle
    etafun = @(d_eta)f(d_eta,d,lambda_u(i),lambda_s(i),tp(i));
    
    caughtwarning = false;
    try
        % Bracket and find d/eta via root of Stone-Holmes first passage time
        bounds = bracketroot(etafun,d_eta0(i),d_etarng,'-');
...
function z=etaroot(d_eta,d,lambda_u,lambda_s,tp,tol)
% Zero of Stone-Holmes first passage time using quadrature integration
lim = d_eta*sqrt(lambda_s);
q = (2/sqrt(pi))*quad(@(x)f(x,d^2*lambda_u,(d/d_eta)^2),0,lim,tol);
z = tp+(erf(lim)*log1p(lambda_u/lambda_s)-q)/(2*lambda_u);


function z=etarootgk(d_eta,d,lambda_u,lambda_s,tp,tol)
% Zero of Stone-Holmes first passage time using quadrature integration
q = (2/sqrt(pi))*quadgk(@(x)f(x,d^2*lambda_u,(d/d_eta)^2),0,Inf,'RelTol',tol,...
    'AbsTol',tol^1.5);
z = tp+(erf(d_eta*sqrt(lambda_s))*log1p(lambda_u/lambda_s)-q)/(2*lambda_u);


function y=f(x,d2lambda_u,eta2)
% Quadrature integration integrand for etaroot() and etarootgk()
y = (log(d2lambda_u+eta2*x.^2)-log(eta2*x.^2)).*exp(-x.^2);


function z=etarootinterp(d_eta,lambda_u,lambda_s,tp)
