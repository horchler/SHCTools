function status=shc_sdephaseplot3(t,y,flag,w)
%SHC_SDEPHASEPLOT3  3-D phase space SDE output function.
%   When the function SHC_SDEPHASEPLOT3 is passed to an SDE solver as the
%   'OutputFUN' property, i.e.,
%   OPTIONS = SHC_SDESET('OutputFUN',@SHC_SDEPHASEPLOT3), the solver calls
%   SHC_SDEPHASEPLOT3(T,Y,'') after every timestep. The SHC_SDEPHASEPLOT3
%   function plots the first three components of the solution, Y, as they are
...
%   calls STATUS = SHC_SDEPHASEPLOT3(T,Y,''). If the solver's 'Refine' property
%   is greater than one (see SHC_SDESET), T is a column vector of new output
%   times and Y is an array of corresponding column vectors. The STATUS return
...
%   vector of indices to pass the integrated Wiener increments to
%   SHC_SDEPHASEPLOT3 as a fourth argument, SHC_SDEPHASEPLOT3(T,Y,'',W). If the
%   'OutputWSelect' property is set to 'yes' or a non-empty vector of indices
%   the solver calls SHC_SDEPHASEPLOT3(TSPAN,Y0,'init',W0) to initialize the
%   output function at the start of integration and
%   SHC_SDEPHASEPLOT3([],[],'done',[]) when the integration is complete.
%   
%   See also:
%       SHC_SDEPLOT, SHC_SDEPRINT, SHC_LV_INTEGRATE, SHC_SDESET, SHC_SDEGET,
%       ODEPHAS3

%   SHC_SDEPHASEPLOT3 is based on an updating of Matlab's ODEPHAS3,
%   version 1.27.4.10
...
switch flag
    case 'init'
        ...
        elseif isW
            ...
            if D+N >= 3
                ...
                error('SHCTools:shc_sdephaseplot3:TooFewInputsW',...
            	     ['Output function requires at least three solution '...
                      'components from Y or W.']);
            end
        else
            error('SHCTools:shc_sdephaseplot3:TooFewInputs',...
            	 ['Output function requires at least three solution '...
                  'components from Y.']);
        end
    ...
    case ''
        if isempty(FIG_HANDLE) || isempty(AX_HANDLE)
            if isW
                error('SHCTools:shc_sdephaseplot3:NotCalledWithInitW',...
                     ['Output function has not been initialized. Use syntax '...
                      'OutputFUN(TSPAN,Y0,''init'',W0).']);
            else
                error('SHCTools:shc_sdephaseplot3:NotCalledWithInit',...
                     ['Output function has not been initialized. Use syntax '...
    ...
    otherwise
        error('SHCTools:shc_sdephaseplot3:InvalidFlag',...
