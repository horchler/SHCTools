function status=shc_sdeplot(t,y,flag,w)
%SHC_SDEPLOT  Time series SDE output function.
%   When the function SHC_SDEPLOT is passed to an SDE solver as the 'OutputFUN'
%   property, i.e., OPTIONS = SHC_SDESET('OutputFUN',@SHC_SDEPLOT), the solver
%   calls SHC_SDEPLOT(T,Y,'') after every timestep. The SHC_SDEPLOT function
%   plots all components of the solution, Y, as they are computed, adapting the
%   axis limits of the plot dynamically. To plot only particular components of
...
%   the solver's 'Refine' property is greater than one (see SHC_SDESET), T is a
...
%   SHC_SDEPLOT(T,Y,'',W). If the 'OutputWSelect' property is set to 'yes' or a
%   non-empty vector of indices the solver calls SHC_SDEPLOT(TSPAN,Y0,'init',W0)
%   to initialize the output function at the start of integration and
%   SHC_SDEPLOT([],[],'done',[]) when the integration is complete.
%   
%   See also:
%       SHC_SDEPHASEPLOT3, SHC_SDEPRINT, SHC_LV_INTEGRATE, SHC_SDESET,
%       SHC_SDEGET, ODEPLOT
...
switch flag
    ...
    case ''
        if isempty(FIG_HANDLE) || isempty(AX_HANDLE)
            if isW
                error('SHCTools:shc_sdeplot:NotCalledWithInitW',...
                     ['Output function has not been initialized. Use syntax '...
                      'OutputFUN(TSPAN,Y0,''init'',W0).']);
            else
                error('SHCTools:shc_sdeplot:NotCalledWithInit',...
                     ['Output function has not been initialized. Use syntax '...
    ...
    otherwise
        error('SHCTools:shc_sdeplot:InvalidFlag',...
